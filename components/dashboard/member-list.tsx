...
export const MemberList: React.FC<MemberListProps> = ({ members = [], loading, totalShares, onDistributeMember }) => {
	...
	React.useEffect(()=>{ if(query) track({ name: 'members_search', queryLength: query.length } as any) },[query, track]);
	...
	return (
		<section className="flex flex-col gap-4" aria-label="Members">
			...
			<div className="glass-panel overflow-hidden" data-elev={2} role="table" aria-label="Member share allocations">
				<div className="grid grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-3 px-4 py-2 text-[11px] uppercase tracking-[1.2px] text-[var(--text-color-muted)] bg-black/10" role="row">
				...
				{!loading && filtered.length===0 && <div className="p-6 text-sm text-muted-foreground" role="row">No members found.</div>}
				{!loading && filtered.map(m=>{ const pct= totalShares ? (m.shares/totalShares)*100 : 0; return (
					<div key={m.id} className="grid grid-cols-[1.5fr_1fr_1fr_1fr_auto] gap-3 px-4 py-2.5 text-sm items-center hover:bg-white/5 transition-colors" role="row">
						<div className="flex items-center gap-2 min-w-0">
							...
							<CopyButton value={m.address} label="Copy Address" className="ml-auto" onCopy={()=>track({ name: 'copied_value', valueType: 'member_address' } as any)} />
						</div>
						<div className="text-right tabular-nums">{m.claimed.toFixed(6)}</div>
						<div className="text-right tabular-nums">{m.shares}</div>
						<div className="text-right tabular-nums" aria-label={`Share percent ${pct.toFixed(2)}%`}>{pct.toFixed(2)}%</div>
						<div className="flex justify-end"><TextureButton size="sm" variant="glass" className="w-auto px-3" onClick={() => { track({ name: 'distribution_initiated', scope: 'member', memberId: m.id } as any); onDistributeMember?.(m); }}>Distribute</TextureButton></div>
					</div>
				) })}
			</div>
