import { emptyWallet, findAta, findMintEditionId, findMintMetadataId, tryGetAccount, withFindOrInitAssociatedTokenAccount, withWrapSol, } from "@cardinal/common";
...
import { getRemainingAccountsForKind, InvalidationType, TOKEN_MANAGER_ADDRESS, TokenManagerKind, tokenManagerProgram, withRemainingAccountsForReturn, } from "./programs/tokenManager";
import { getTokenManager } from "./programs/tokenManager/accounts";
import { findMintManagerId, findTokenManagerAddress, findTransferReceiptId, } from "./programs/tokenManager/pda";
import { transferAuthorityProgram, WSOL_MINT, } from "./programs/transferAuthority";
import { getListing, getMarketplace, } from "./programs/transferAuthority/accounts";
...
export const withInitMarketplace = async (transaction, connection, wallet, name, paymentManagerName, paymentMints, payer = wallet.publicKey) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
...
export const withUpdateMarketplace = async (transaction, connection, wallet, name, paymentManagerName, authority, paymentMints) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
    const marketplaceId = findMarketplaceAddress(name);
    const paymentManagerId = findPaymentManagerAddress(paymentManagerName);
...
export const withCreateListing = async (transaction, connection, wallet, mintId, markeptlaceName, paymentAmount, paymentMint = PublicKey.default, payer = wallet.publicKey) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
    ...
    const checkListing = await tryGetAccount(() => getListing(connection, listingId));
    if (checkListing) {
        transaction.add(await withUpdateListing(transaction, connection, wallet, mintId, marketplaceId, paymentAmount, paymentMint));
    }
    else {
...
export const withUpdateListing = async (transaction, connection, wallet, mintId, marketplaceId, paymentAmount, paymentMint) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
    ...
    const listerMintTokenAccountId = await findAta(mintId, wallet.publicKey, true);
    const listingId = findListingAddress(mintId);
...
export const withRemoveListing = async (transaction, connection, wallet, mintId, listerMintTokenAccountId) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
    const listingId = findListingAddress(mintId);
    const tokenManagerId = findTokenManagerAddress(mintId);
    const mintManagerId = findMintManagerId(mintId);
...
export const withAcceptListing = async (transaction, connection, wallet, buyer, mintId, paymentAmount, paymentMint, buySideReceiver, payer = buyer) => {
    const transferAuthProgram = transferAuthorityProgram(connection, wallet);
    const listingData = await tryGetAccount(() => getListing(connection, mintId));
    if (!(listingData === null || listingData === void 0 ? void 0 : listingData.parsed)) {
    ...
    const listerPaymentTokenAccountId = nativePayment
        ...
        : await withFindOrInitAssociatedTokenAccount(transaction, connection, listingData.parsed.paymentMint, listingData.parsed.lister, wallet.publicKey);
    ...
    const payerPaymentTokenAccountId = nativePayment
        ...
        : listingData.parsed.lister.toString() !== payer.toString()
            ? await withFindOrInitAssociatedTokenAccount(transaction, connection, listingData.parsed.paymentMint, payer, wallet.publicKey)
    ...
    const buyerMintTokenAccountId = listingData.parsed.lister.toString() === buyer.toString()
        ...
        : await withFindOrInitAssociatedTokenAccount(transaction, connection, mintId, buyer, wallet.publicKey, true);
    const feeCollectorTokenAccountId = nativePayment
        ...
        : await withFindOrInitAssociatedTokenAccount(transaction, connection, listingData.parsed.paymentMint, paymentManagerData === null || paymentManagerData === void 0 ? void 0 : paymentManagerData.parsed.feeCollector, wallet.publicKey, true);
    const mintMetadataId = findMintMetadataId(mintId);
    const tokenManagerId = findTokenManagerAddress(mintId);
    const transferReceiptId = findTransferReceiptId(tokenManagerId);
    const remainingAccountsForHandlePaymentWithRoyalties = await withRemainingAccountsForHandlePaymentWithRoyalties(transaction, connection, wallet, mintId, listingData.parsed.paymentMint, buySideReceiver, [listingData.parsed.lister.toString(), buyer.toString()]);
    ...
    const remainingAccountsForKind = getRemainingAccountsForKind(mintId, tokenManagerData.parsed.kind);
...
export const withWhitelistMarektplaces = async (transaction, connection, wallet, transferAuthorityName, marketplaceNames) => {
    ...
    const marketplaceIds = marketplaceNames.map((name) => findMarketplaceAddress(name));
...
export const withRelease = async (transaction, connection, wallet, mintId, transferAuthorityId, holderTokenAccountId, payer = wallet.publicKey) => {
    ...
    const tokenManagerTokenAccountId = await withFindOrInitAssociatedTokenAccount(transaction, connection, mintId, tokenManagerId, payer, true);
    const tokenManagerData = await getTokenManager(connection, tokenManagerId);
    const remainingAccountsForKind = getRemainingAccountsForKind(mintId, tokenManagerData.parsed.kind);
